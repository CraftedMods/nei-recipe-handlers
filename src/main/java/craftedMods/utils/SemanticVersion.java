...
public class SemanticVersion implements Comparable<SemanticVersion> {
	...
	public boolean equals(Object obj) {
		...
		if (this.patchVersion != other.patchVersion) return false;
		if (this.preReleaseVersion != other.preReleaseVersion) return false;
		if (this.versionState != other.versionState) return false;
		return true;
	}
	...
	private String buildVersionString() {
		return String.format("%d.%d.%d%s%s", this.majorVersion, this.minorVersion, this.patchVersion,
				...
				this.preReleaseVersion >= 0 ? String.format("%s.%d", this.versionState == EnumVersionState.FULL ? "-PRE" : "", this.preReleaseVersion) : "");
	}
