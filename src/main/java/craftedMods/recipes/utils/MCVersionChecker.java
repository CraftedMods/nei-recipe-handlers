...
public class MCVersionChecker {
	...
	private final ModVersion currentVersion = new ModVersion(EnumVersionState.ALPHA, NEIRecipeHandlers.MODID, NEIRecipeHandlers.MODNAME,
			NEIRecipeHandlers.VERSION, MinecraftForge.MC_VERSION);
	...
	public MCVersionChecker(String versionFileURL) throws MalformedURLException {
		...
		try {
			...
			NEIRecipeHandlers.mod.getLogger().error("The URL of the version file (\"" + versionFileURL + "\") isn't valid!");
			throw e;
		}
	...
	private ModVersion parseVersionString(String versionString) throws MalformedURLException {
		...
		NEIRecipeHandlers.mod.getLogger().info("Found version: " + version);

		return new ModVersion(state, NEIRecipeHandlers.MODID, NEIRecipeHandlers.MODNAME, version, mcVersion, url);
	}
	...
	public boolean ping() throws Exception {
		try {
			...
			NEIRecipeHandlers.mod.getLogger().error("Cannot connect to the version file. Do you have an internet connection?");
			throw e;
		}
