...
public class NEIRecipeHandlersUtils
{
    ...
    public static ItemStack[] extractRecipeItems (Object container)
    {
        ...
        {
            ret = new ItemStack[]
            {new ItemStack ((Item) container)};
        }
        else if (container instanceof Block)
        {
            ret = new ItemStack[]
            {new ItemStack ((Block) container)};
        }
        else
        {
    ...
    {
        return ((List) ReflectionHelper.getPrivateValue (Minecraft.class, Minecraft.getMinecraft (),
            "defaultResourcePacks", "field_110449_ao")).add (pack);
    }

    @SuppressWarnings("unchecked")
    public static <T> Collection<T> discoverRegisteredHandlers (
        ...
        Class<T> handlerClass)
    {
        ...
        NEIRecipeHandlers.mod.getLogger ()
            .info (String.format ("Found %d handlers of type \"%s\" in the classpath", discoveredHandlers.size (),
        ...
        discoveredHandlers.forEach (clazz ->
        {
            try
            {
                ...
                if (handler.getAnnotation (RegisteredHandler.class).isEnabled ())
                {
                    ...
                    NEIRecipeHandlers.mod.getLogger ()
                        .debug (String.format ("Registered the handler \"%s\" of type \"%s\"", handler.getName (),
                            handlerClass.getSimpleName ()));
                }
                else
                {
                    NEIRecipeHandlers.mod.getLogger ().info (
                        String.format ("The handler \"%s\" of type \"%s\" was disabled by the author",
                            handler.getName (), handlerClass.getSimpleName ()));
                }
            }
            catch (Exception e)
            {
                NEIRecipeHandlers.mod.getLogger ()
                    .error ("Couldn't create an instance of class \"" + clazz.getName () + "\"", e);
            }
        });
    ...
    {
        IChatComponent part1 = new ChatComponentTranslation ("neiRecipeHandlers.versionChecker.notification.chat.part1",
            handlerName)
        ...
        {
            IChatComponent part4 = new ChatComponentTranslation (
                "neiRecipeHandlers.versionChecker.notification.chat.part4")
                    .setChatStyle (new ChatStyle ().setColor (EnumChatFormatting.WHITE));
            IChatComponent part5 = new ChatComponentTranslation (
                "neiRecipeHandlers.versionChecker.notification.chat.part5")
                        ...
                        .setChatHoverEvent (new HoverEvent (net.minecraft.event.HoverEvent.Action.SHOW_TEXT,
                            new ChatComponentText (
                                StatCollector.translateToLocalFormatted (
                                    "neiRecipeHandlers.versionChecker.notification.chat.changelog.tooltip")))));
            IChatComponent part6 = new ChatComponentTranslation (
                "neiRecipeHandlers.versionChecker.notification.chat.part6")
                    .setChatStyle (new ChatStyle ().setColor (EnumChatFormatting.WHITE));
