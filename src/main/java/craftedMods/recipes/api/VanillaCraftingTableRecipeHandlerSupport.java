...
public interface VanillaCraftingTableRecipeHandlerSupport {
	...
	/**
	 * Invoked if a recipe type was found which couldn'd be processed. Either return the processed recipe instance(s) or an empty collection or null,
	 * which means that the recipe will be sent to the other handlers, which try to process it. The second argument of the pair is whether the recipe
	 * should be ignored, meaning that it won't be sent to other handlers, but also that no message will be logged. The returned recipe instance of
	 * not relevant then. Usually used if the recipe is computed some other way. If null is returned, the same thing happens as if the processed
	 * instance is null and the second argument is false.
	 * 
