...
public interface RecipeHandlerRendererUtils {
	...
	/**
	 * Draws a rectangle from P(x|y) to Q(x+width|y+height) with a gradient color (from color1 to color2)
	 * 
	 * @param x The x origin
	 * @param y The y origin
	 * @param width The width
	 * @param height The height
	 * @param color1 The left color bound
	 * @param color2 The right color bound
	 */
	public void drawGradientRectangle(int x, int y, int width, int height, int color1, int color2);

	/**
	 * Draws a rectangle from P(x|y) to Q(x+width|y+height) with a part of the current bound texture from R(textureX|textureY) to
	 * S(textureX+width|textureHeight+height)
	 * 
	 * @param x The x origin
	 * @param y The y origin
	 * @param textureX The x (u) coordinate in the texture
	 * @param textureY The y (v) coordinate inF the texture
	 * @param width The width
	 ...
	 * @param shadow Whether a text shadow should be drawn
	 */
	...
	/**
	 * Draws a progress bar from P(x|y) to Q(x+width|y+height) with a part of the current bound texture from R(textureX|textureY) to
	 * S(textureX+width|textureHeight+height) by the provided completion (100% for the whole bar) from the provided direction.
	 * 
	 * @param x The x origin
	 * @param y The x origin
	 * @param textureX The x (u) coordinate in the texture
	 * @param textureY The y (v) coordinate in the texture
	 * @param width The width
	 * @param height The height
	 * @param completion The completion factor (1.0: 100%)
	 * @param direction The direction from/to which the progress bar grows/shrinks
	 */
