...
public class RecipeHandlerManager {
	...
	private void registerRecipeHandler(RecipeHandler<?> instance) {
		if (this.recipeHandlers.putIfAbsent(instance.getUnlocalizedName(), instance) == null) NEIRecipeHandlers.mod.getLogger()
				.debug("Successfully registered recipe handler \"" + instance.getUnlocalizedName() + "\" of class \"" + instance.getClass().getName() + "\"");
		else NEIRecipeHandlers.mod.getLogger().warn("Couldn't register recipe handler \"" + instance.getClass().getName()
	...
	private void loadRecipesFromCache(Map<RecipeHandler<?>, Collection<Recipe>> cachedRecipes) {
		if (NEIRecipeHandlers.mod.getConfig().isComplicatedStaticRecipeLoadingCacheEnabled())
			...
			} else NEIRecipeHandlers.mod.getLogger().info("The recipe handler \"" + handler.getUnlocalizedName() + "\" loaded no recipes from the cache");
			} else NEIRecipeHandlers.mod.getLogger().info("The cache of the recipe handler \"" + key + "\" is not valid, the discovered data won't be used");
			} else
	...
	private void loadStaticRecipes(Map<RecipeHandler<?>, Collection<Recipe>> staticRecipes) {
		...
		if (maxItemIterationDepth > 2) NEIRecipeHandlers.mod.getLogger().warn("The static recipe iteration depth (" + maxItemIterationDepth
	...
	private void writeRecipesToCache() {
		if (NEIRecipeHandlers.mod.getConfig().isComplicatedStaticRecipeLoadingCacheEnabled())
			...
			NEIRecipeHandlers.mod.getLogger().error("The recipe handler \"" + handler.getUnlocalizedName() + "\" couldn't write data to the cache: ", e);
			}
