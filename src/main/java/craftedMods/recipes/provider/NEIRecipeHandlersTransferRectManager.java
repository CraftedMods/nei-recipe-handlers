...
public class NEIRecipeHandlersTransferRectManager
    implements IContainerInputHandler, IContainerTooltipHandler, IContainerDrawHandler
{
    ...
    public static void registerHandler (RecipeHandler<?> handler)
    {
        if (handler.getRecipeViewer () != null)
        {
            ...
            {
                Collection<Class<? extends GuiContainer>> guiClasses = new ArrayList<> (
                    recipeViewer.getSupportedGUIClasses ());
                ...
                for (Class<? extends GuiContainer> guiClass : guiClasses)
                {
                    ...
                    NEIRecipeHandlersTransferRectManager.guiMap.get (guiClass).put (handler,
                        new TransferRect (area,
                            NEIRecipeHandlersTransferRectManager.getViewAllRecipedIdentifier (handler), new Object[0]));
                }
            }
        }
    ...
    {
        return NEIRecipeHandlersTransferRectManager.guiMap.containsKey (gui.getClass ())
            && NEIRecipeHandlersTransferRectManager.guiMap.get (gui.getClass ())
    ...
    public boolean lastKeyTyped (GuiContainer gui, char keyChar, int keyCode)
    {
        ...
        if (gui instanceof GuiRecipe)
        {
            ...
            Boolean res = this.executeForGuiRecipe (guiRecipe, null, (triple) ->
            {
                ...
                {
                    result = transferRect (gui, triple.getRight (), offset.x, offset.y, false);
                }
                if (keyCode == NEIClientConfig.getKeyBinding ("gui.usage"))
                {
                    result = transferRect (gui, triple.getRight (), offset.x, offset.y, true);
                }
        ...
        else
        {
            ...
            if (keyCode == NEIClientConfig.getKeyBinding ("gui.recipe"))
                return transferRect (gui, null, offset[0], offset[1], false);
            if (keyCode == NEIClientConfig.getKeyBinding ("gui.usage"))
                return transferRect (gui, null, offset[0], offset[1], true);
        }
    ...
    private boolean transferRect (GuiContainer gui, RecipeHandler<?> currentHandler, int offsetx, int offsety,
        boolean usage)
    {
        Map<RecipeHandler<?>, TransferRect> transferRects = NEIRecipeHandlersTransferRectManager.guiMap
            .get (gui.getClass ());
        ...
        {
            if (entry.getKey ().getRecipeViewer ().isGuiContainerSupported (gui)
                && (currentHandler != null ? currentHandler == entry.getKey () : true))
            {
                TransferRect rect = entry.getValue ();
                if (!rect.rect.contains (relMouse)
                    || ! (usage ? GuiUsageRecipe.openRecipeGui (rect.outputId, rect.results)
    ...
    private List<String> transferRectTooltip (GuiContainer gui, RecipeHandler<?> currentHandler,
        Map<RecipeHandler<?>, TransferRect> transferRects, int offsetx,
        ...
        if (GuiContainerManager.shouldShowTooltip (gui) && currenttip.size () == 0)
        {
            ...
            if (gui instanceof GuiRecipe)
            {
                ...
                ret = this.executeForGuiRecipe (guiRecipe, null, (triple) ->
                {
                    ...
                    List<String> newTip = transferRectTooltip (gui, triple.getRight (),
                        NEIRecipeHandlersTransferRectManager.guiMap.get (gui.getClass ()),
            ...
            {
                int[] offset = RecipeInfo.getGuiOffset (gui);
                ret = transferRectTooltip (gui, null,
                    NEIRecipeHandlersTransferRectManager.guiMap.get (gui.getClass ()), offset[0], offset[1],
                    currenttip);
            }
        }
        return ret;
            ...
            NEIRecipeHandlersTransferRectManager.guiMap.get (guiContainer.getClass ()).forEach ( (handler, rect) ->
            {
                if (handler.getRecipeViewer ().isGuiContainerSupported (guiContainer))
                {
                    ...
                    if (guiContainer instanceof GuiRecipe)
                    {
                        ...
                        {
                            Point offset = guiRecipe.getRecipePosition (triple.getLeft ());
                            renderButtons (guiContainer, handler, offset.x, offset.y);
                            return null;
                        }, true);
                    ...
                    else
                    {
                        ...
                        renderButtons (guiContainer, handler, offset[0], offset[1]);
                    }
        ...
        Function<Triple<Integer, RecipeHandler<?>, RecipeHandler<?>>, T> toExecute, boolean innerHandlerEqualToHandler)
    {
        ...
        if (currentHandler instanceof PluginRecipeHandler<?, ?>)
        {
            ...
            if (innerHandlerEqualToHandler ? innerHandler == handler : true)
            {
                ...
                for (int currentRecipe = guiRecipe.page
                    * currentHandler.recipiesPerPage (); currentRecipe < currentHandler.numRecipes ()
    ...
    private void renderButtons (GuiContainer guiContainer, RecipeHandler<?> handler, int offsetX, int offsetY)
    {
        ...
        Point relMouse = new Point (absMousePos.x - guiContainer.guiLeft - offsetX,
            absMousePos.y - guiContainer.guiTop - offsetY);

        Rectangle location = new Rectangle (RecipeHandlerRecipeViewer.VIEW_ALL_RECIPES_RECTANGLE);
        location.translate (handler.getRecipeViewer ().getOffsetX (guiContainer.getClass ()),
            handler.getRecipeViewer ().getOffsetY (guiContainer.getClass ()));
        ...
        // Render the book item
        RenderItem.getInstance ().renderItemIntoGUI (GuiDraw.fontRenderer, GuiDraw.renderEngine,
            handler.getRecipeViewer ().getButtonIcon (guiContainer.getClass ()),
            0, 0);
    ...
    public static class TransferRect
    {
        ...
        public boolean equals (Object obj)
        {
            if (! (obj instanceof TransferRect))
                return false;
            return rect.equals ( ((TransferRect) obj).rect);
        }
